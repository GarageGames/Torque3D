...
/**
 Documentation: 
  * This control is very simple to use, and it follows the basis of a non-wrapped GuiBitmapControl
  * You define a image for the object to use, and can then define a number of images to string together.
  * I have also added a "cutoff" feature, where you can define a second image and a cutoff point to where
  * the control will switch to using the cutoff image instead!

  * Engine Function Declarations:
  setBitmap() - Define the bitmap for the control
  setCutoffBitmap() - Define the cutoff bitmap for the control
  setImageCount() - Define the number of images to string together
  setCutoffPoint() - Set the point to where cutoff begins.
