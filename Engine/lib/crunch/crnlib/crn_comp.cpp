...
namespace crnlib
{
      ...
      uint trial_index)
   {
      ...
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"pack_color_endpoints: %u", trial_index);
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Total endpoint residuals: %i", total_residuals);
#endif
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES               
         if (m_pParams->m_flags & cCRNCompFlagDebugging)
            console::debug(L"Total endpoint index delta: %I64i", total_delta);
#endif            
      }

#if CRNLIB_CREATE_DEBUG_IMAGES      
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"color_endpoint_residuals_%u.tga", trial_index).get_ptr(), endpoint_residual_image);
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"color_endpoints_%u.tga", trial_index).get_ptr(), endpoint_image);
#endif   
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for color endpoint residual Huffman tables", codec.encode_get_total_bits_written());
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for color endpoint residuals", codec.encode_get_total_bits_written() - start_bits);
#endif      
      ...
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
      {
         console::debug(L"Wrote a total of %u bits for color endpoint codebook", codec.encode_get_total_bits_written());

         console::debug(L"Wrote %f bits per each color endpoint", data.size() * 8.0f / m_hvq.get_color_endpoint_codebook_size());
      }         
#endif      
      ...
      uint trial_index)
   {
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)   
         console::debug(L"pack_alpha_endpoints: %u", trial_index);
#endif         
...
#if CRNLIB_CREATE_DEBUG_IMAGES      
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"alpha_endpoint_residuals_%u.tga", trial_index).get_ptr(), endpoint_residual_image);
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"alpha_endpoints_%u.tga", trial_index).get_ptr(), endpoint_image);
#endif   
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for alpha endpoint residual Huffman tables", codec.encode_get_total_bits_written());
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for alpha endpoint residuals", codec.encode_get_total_bits_written() - start_bits);
#endif         
      ...
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
      {
         console::debug(L"Wrote a total of %u bits for alpha endpoint codebook", codec.encode_get_total_bits_written());

         console::debug(L"Wrote %f bits per each alpha endpoint", data.size() * 8.0f / m_hvq.get_alpha_endpoint_codebook_size());
      }         
#endif      
      ...
      uint trial_index)
   {
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"pack_selectors: %u", trial_index);
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES            
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Total selector endpoint residuals: %u", total_residuals);
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES               
         if (m_pParams->m_flags & cCRNCompFlagDebugging)
            console::debug(L"Total selector index delta: %I64i", total_delta);
#endif            
      }      

#if CRNLIB_CREATE_DEBUG_IMAGES      
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"selectors_%u_%u.tga", trial_index, max_selector_value).get_ptr(), selector_image);
      image_utils::save_to_file(dynamic_wstring(cVarArg, L"selector_residuals_%u_%u.tga", trial_index, max_selector_value).get_ptr(), residual_image);
#endif   
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for selector residual Huffman tables", codec.encode_get_total_bits_written());
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"Wrote %u bits for selector residuals", codec.encode_get_total_bits_written() - start_bits);
#endif         
      ...
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
      {
         console::debug(L"Wrote a total of %u bits for selector codebook", codec.encode_get_total_bits_written());

         console::debug(L"Wrote %f bits per each selector codebook entry", packed_data.size() * 8.0f / selectors.size());
      }         
#endif      
   ...
   bool crn_comp::quantize_chunks()
   {
      ...
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
      {
         console::debug(L"Color endpoints: %u", params.m_color_endpoint_codebook_size);
         console::debug(L"Color selectors: %u", params.m_color_selector_codebook_size);
         console::debug(L"Alpha endpoints: %u", params.m_alpha_endpoint_codebook_size);
         console::debug(L"Alpha selectors: %u", params.m_alpha_selector_codebook_size);
      }
      ...
      if (params.m_debugging)
      {
         ...
         image_utils::save_to_file(L"quantized_chunks.tga", img);
      }
#endif   
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"----- Begin optimization of color endpoint codebook");
#endif   
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES               
         if (m_pParams->m_flags & cCRNCompFlagDebugging)
            console::debug(L"Pack chunks simulation: %u bits", total_packed_chunk_bits);
#endif            
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES               
         if (m_pParams->m_flags & cCRNCompFlagDebugging)
            console::debug(L"Total bits: %u", total_bits);
#endif            
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES            
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"End optimization of color endpoint codebook");
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES            
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"----- Begin optimization of color selector codebook");
#endif
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES            
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"End optimization of color selector codebook");
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"----- Begin optimization of alpha endpoint codebook");
#endif 
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES            
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"End optimization of alpha endpoint codebook");
#endif         
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"----- Begin optimization of alpha selector codebook");
#endif
...
#if CRNLIB_ENABLE_DEBUG_MESSAGES      
      if (m_pParams->m_flags & cCRNCompFlagDebugging)
         console::debug(L"End optimization of alpha selector codebook");
#endif         
