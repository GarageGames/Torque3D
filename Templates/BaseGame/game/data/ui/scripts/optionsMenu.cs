...
function AudioMenu::loadSettings(%this)
{
   ...
   MasterVolumeSlider.setValue( $pref::SFX::masterVolume );
   GuiVolumeSlider.setValue( $pref::SFX::channelVolume[ $GuiAudioType] );
   EffectVolumeSlider.setValue( $pref::SFX::channelVolume[ $SimAudioType ] );
   MusicVolumeSlider.setValue( $pref::SFX::channelVolume[ $MusicAudioType ] );
   ...
   for(%i = 0; %i < %count; %i++)
   {
      ...
      //If we have legitimate devices, we don't need to list the null device
      if(%count > 1 && %provider $= "Null")
         continue;
